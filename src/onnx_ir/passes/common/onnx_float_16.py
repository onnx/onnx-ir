...
def _npfloat16_to_int(np_list: list[np.float16] | npt.NDArray[np.float16]) -> list[int]:
...
def convert_np_to_float16(
    np_array: npt.NDArray[np.float32],
    ...
    def between(
        a: npt.NDArray | int | float,
        b: npt.NDArray | int | float,
        c: npt.NDArray | int | float,
...
def make_value_info_from_tensor(
    tensor: onnx_proto.TensorProto,
...
def convert_float_to_float16(
    ...
    # create blocklists
    if op_blocks is None:
        op_blocks = DEFAULT_OP_BLOCK_LIST
    if node_blocks is None:
        node_blocks = []
    op_block_list = set(op_blocks)
    node_block_list = set(node_blocks)
    ...
    if keep_io_types:
        ...
        for i, n in enumerate(model.graph.output):
            if n.type.tensor_type.elem_type == onnx_proto.TensorProto.FLOAT:
                ...
                # add Cast node (from tensor(float16) to tensor(float) before
                # graph output
                new_value_info = model.graph.value_info.add()
                new_value_info.CopyFrom(n)
    ...
    while queue:
        ...
        for q in queue:
            ...
            if isinstance(q, onnx_proto.GraphProto):
                for n in q.node:
                    # if n is in the block list (doesn't support float16), no
                    # conversion for the node,
                    ...
                    # don't add the attr into next_level for the node
                    # in node_keep_data_type_list
            ...
            # if q is graph, process graph.initializer(TensorProto), input, output
            # and value_info (ValueInfoProto)
            if isinstance(q, onnx_proto.GraphProto):
                ...
                # for all ValueInfoProto with tensor(float) type in input, output
                # and value_info, convert them to
                # tensor(float16) except map and seq(map). And save them in
                # value_info_list for further processing
    ...
    for node in node_list:
        # if input's name is in the value_info_list meaning input is tensor(float16)
        # type, insert a float16 to float Cast node before the node,
        ...
        for i in range(len(node.input)):
            ...
            for value_info in value_info_list:
                if input == value_info.name:
                    ...
                    graph = node_dict[
                        node.name
                    ]  # get the correct graph instead of the global graph
                    new_value_info = graph.value_info.add()
                    new_value_info.CopyFrom(value_info)
                    output_name = node.name + "_input_cast_" + str(i)
                    new_value_info.name = output_name
                    new_value_info.type.tensor_type.elem_type = onnx_proto.TensorProto.FLOAT
                    # add Cast node (from tensor(float16) to tensor(float)
                    # before current node
                    node_name = node.name + "_input_cast" + str(i)
                    new_node = [
                        helper.make_node("Cast", [input], [output_name], to=1, name=node_name)
                    ]
                    graph.node.extend(new_node)
                    # change current node's input name
                    node.input[i] = output_name
                    break
        # if output's name is in the value_info_list meaning output is
        # tensor(float16) type, insert a float to
        # float16 Cast node after the node, change current node's output name and
        # create new value_info for the new name
        for i in range(len(node.output)):
            ...
            for value_info in value_info_list:
                if output == value_info.name:
                    ...
                    new_value_info.name = input_name
                    new_value_info.type.tensor_type.elem_type = onnx_proto.TensorProto.FLOAT
                    # add Cast node (from tensor(float) to tensor(float16) after
                    # current node
                    node_name = node.name + "_output_cast" + str(i)
                    new_node = [
                        helper.make_node("Cast", [input_name], [output], to=10, name=node_name)
                    ]
                    graph.node.extend(new_node)
                    # change current node's input name
...
def sort_graph_node(graph_proto: onnx_proto.GraphProto) -> None:
    ...
    def find_first_node(
        output2node_dict: dict[str, onnx_proto.NodeProto],
    ) -> onnx_proto.NodeProto | None:
    ...
    def remove_first_node_from_dict2(first_node: onnx_proto.NodeProto) -> None:
        for output in first_node.output:
            output2node_dict.pop(output, None)
