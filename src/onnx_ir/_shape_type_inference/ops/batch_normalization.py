...
class BatchNormalizationInferrer(_common.NodeInferrer):
    ...
    def infer(self, node: ir.Node) -> _common.InferenceResult:
        ...
        if input_shape is None:
            return _common.InferenceResult(failure="BatchNormalization input shape is not known.")
        if scale_shape is None:
            return _common.InferenceResult(failure="BatchNormalization scale shape is not known.")
        if bias_shape is None:
            return _common.InferenceResult(failure="BatchNormalization bias shape is not known.")
        if mean_shape is None:
            return _common.InferenceResult(failure="BatchNormalization mean shape is not known.")
        if var_shape is None:
            return _common.InferenceResult(failure="BatchNormalization var shape is not known.")

        input_rank = len(input_shape)
        if input_rank < 2:
            return _common.InferenceResult(failure="BatchNormalization input must have at least 2 dimensions.")
        ...
        for param_name, param_shape in [("scale", scale_shape), ("bias", bias_shape), 
                                       ("mean", mean_shape), ("var", var_shape)]:
