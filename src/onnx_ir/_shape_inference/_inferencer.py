...
def get_dim_from_proto(dim):
    ...
    return (
        getattr(dim, dim.WhichOneof("value")) if type(dim.WhichOneof("value")) is str else None
    )  # noqa: E721
...
class SymbolicShapeInference:
    ...
    def _add_suggested_merge(self, symbols, apply=False):
        ...
        """
        assert all(
            (type(s) == str and s in self.symbolic_dims_) or is_literal(s) for s in symbols
        )
        symbols = set(symbols)
        ...
        if not skip_infer:
            ...
            if (
                ...
                for d in range(
                    ...
                    in_dims = [
                        s[len(s) - out_rank + d] for s in in_shapes if len(s) + d >= out_rank
                    ]
                    if len(in_dims) > 1:
    ...
    def _onnx_infer_subgraph(self, node, subgraph, use_node_input=True, inc_subgraph_id=True):
        ...
        tmp_graph.initializer.extend(
            [i for i in self.out_mp_.graph.initializer if i.name in subgraph_implicit_input]
        )
        ...
        subgraph_shapes = [
            get_shape_from_value_info(o) for o in symbolic_shape_inference.out_mp_.graph.output
        ]
    ...
    def _infer_aten_upsample(self, node):
        ...
        if input_shape is not None:
            ...
            if output_size is not None:
                ...
                new_shape += [
                    str(self._new_symbolic_dim_from_output(node, 0, i)) for i in range(2, rank)
                ]
        if node.output[0] and new_shape is not None:
            output_dtype = self.known_vi_[node.input[0]].type.tensor_type.elem_type
            vi = self.known_vi_[node.output[0]]
    ...
    def _infer_Slice(self, node):  # noqa: N802
        ...
        def flatten_min(expr):
            ...
            min_positions = [
                idx for idx in range(len(expr.args)) if isinstance(expr.args[idx], sympy.Min)
            ]
        ...
        for i_o in range(len(split)):
            ...
            vi.CopyFrom(
                make_value_info_func(
                    ...
                    self.known_vi_[node.input[0]].type.tensor_type.elem_type,
                    get_shape_from_sympy_shape(
                        input_sympy_shape[:axis] + [split[i_o]] + input_sympy_shape[axis + 1 :]
                    ),
                )
            )
    ...
    def _infer_Transpose(self, node):  # noqa: N802
        ...
        if node.input[0] in self.sympy_data_:
            ...
            self.sympy_data_[node.output[0]] = (
                np.transpose(np.array(input_data).reshape(*data_shape), axes=tuple(perm))
                .flatten()
                .tolist()
            )
    ...
    def _infer_Attention(self, node):  # noqa: N802
        ...
        if shape and len(shape) == 3:
            ...
            if len(node.output) > 1:
                ...
                input_shape = self._get_shape(node, 0)
                past_shape = (
                    self._get_shape(node, 4) if len(node.input) > 4 and node.input[4] else []
                )
                mask_shape = (
                    self._get_shape(node, 3) if len(node.input) > 3 and node.input[3] else []
                )
                ...
                if past_shape and len(past_shape) == 5:
                    ...
                    vi = self.known_vi_[node.output[1]]
                    vi.CopyFrom(
                        helper.make_tensor_value_info(vi.name, output_dtype, present_shape)
                    )
    ...
    def _infer_RemovePadding(self, node):  # noqa: N802
        ...
        if shape and len(shape) == 3:
            ...
            vi_max_seq_len.CopyFrom(
                helper.make_tensor_value_info(node.output[3], onnx.TensorProto.INT32, [1])
            )
    ...
    def _infer_LayerNormalization(self, node):  # noqa: N802
        ...
        if len(node.output) > 1:
            ...
            if x_shape is not None:
                ...
                vi = self.known_vi_[node.output[1]]
                vi.CopyFrom(
                    helper.make_tensor_value_info(node.output[1], mean_dtype, mean_shape)
                )
                if len(node.output) > 2:
                    vi = self.known_vi_[node.output[2]]
                    vi.CopyFrom(
                        helper.make_tensor_value_info(node.output[2], mean_dtype, mean_shape)
                    )
    ...
    def _infer_EmbedLayerNormalization(self, node):  # noqa: N802
        ...
        vi = self.known_vi_[node.output[0]]
        vi.CopyFrom(
            helper.make_tensor_value_info(node.output[0], word_embedding_dtype, output_shape)
        )
    ...
    def _infer_PythonOp(self, node):  # noqa: N802
        ...
        assert output_tensor_types, (
            f"PythonOp '{node.name}' has no output_tensor_types attribute."
        )
        output_tensor_ranks = get_attribute(node, "output_tensor_ranks")
        assert output_tensor_ranks, (
            f"PythonOp '{node.name}' has no output_tensor_ranks attribute."
        )
        ...
        if shape_inferer is not None:
            ...
            for i in range(len(node.output) - 1):
                ...
                vi.CopyFrom(
                    helper.make_tensor_value_info(
                        node.output[output_index], output_dtypes[i], output_shapes[i]
                    )
                )
        else:
    ...
    def _propagate_shape_and_type(self, node, input_index=0, output_index=0):
        ...
        vi = self.known_vi_[node.output[output_index]]
        vi.CopyFrom(
            helper.make_tensor_value_info(node.output[output_index], output_dtype, shape)
        )
    ...
    def _infer_impl(self, start_sympy_data=None):
        ...
        def get_prereq(node):
            ...
            for g in subgraphs:
                ...
                for n in g.node:
                    g_prereq.update(
                        [i for i in get_prereq(n) if i not in g_outputs_and_initializers]
                    )
        ...
        sorted_known_vi = {
            i.name
            for i in list(self.out_mp_.graph.input) + list(self.out_mp_.graph.initializer)
        }
