...
def infer_nhwc_conv_shape(node: ir.Node, shape_env: ShapeEnv) -> InferenceResult:
    """Shape inference for NhwcConv op."""
    ...
    if not all(isinstance(dim, int) for dim in input_shape):
        return InferenceResult(InferenceStatus.UNSUPPORTED, "Symbolic dimensions not supported for NhwcConv yet.")
    ...
    if not kernel_shape and len(node.inputs) > 1:
        ...
        kernel_shape = list(weight_shape)[2:] # Assuming HWC format for kernel
    ...
    output_shape[3] = list(shape_env.get_shape(node.inputs[1]))[3] # Output channels

    for i in range(len(kernel_shape)):
        effective_kernel_size = dilations[i] * (kernel_shape[i] - 1) + 1
        padded_input_size = input_shape[i + 1] + pads[i] + pads[i + len(kernel_shape)]
        output_shape[i + 1] = (padded_input_size - effective_kernel_size) // strides[i] + 1
