...
def infer_aten_upsample_shape(node: ir.Node, shape_env: ShapeEnv) -> InferenceResult:
    """Shape inference for ATen upsample ops."""
    ...
    # The output size can be provided as a constant input (sizes) or inferred from scales.
    # For now, we'll assume the output spatial dimensions become dynamic (None).
    output_shape = input_shape[:-len(node.inputs[1].shape)] + [None] * len(node.inputs[1].shape)
