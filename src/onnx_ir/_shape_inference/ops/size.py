...
def infer_size_shape(node: ir.Node, shape_env: ShapeEnv) -> InferenceResult:
    """Shape inference for Size op."""
    ...
    if input_shape.is_static():
        node.outputs[0].const_value = ir.Tensor(np.array(input_shape.size, dtype=np.int64))
